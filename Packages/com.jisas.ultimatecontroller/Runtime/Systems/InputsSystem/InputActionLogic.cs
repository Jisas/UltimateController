...
namespace UltimateFramework.Inputs
{
    ...
    public class InputActionLogic
    {
            ...
            if (Input.action.type == InputActionType.Button)
            {
                if (UseTwoActionsOnInput)
                {
                    ...
                    if (inputValue) {
                        ...
                        ExecuteAction(PrimaryAction.actionTag.tag, PrimaryAction.priority, FindCurrentWeapon(), PrimaryAction.isBaseAction);
                    }
                    else {
                        PressCount++;
                        logicExtension?.Invoke();
                        ExecuteAction(SecondaryAction.actionTag.tag, SecondaryAction.priority, FindCurrentWeapon(), SecondaryAction.isBaseAction);
                    }
                }
                else
                {
        ...
        public void ExecuteAction(string tag, ActionsPriority priority, string weaponName, bool isBaseAction)
        {
            if (InputsManager.Player.enabled)
            {
                if (isBaseAction) EntityInputs.ExecuteBaseAction.Invoke(tag, priority, weaponName);
                else EntityInputs.ExecuteSpecificAction.Invoke(tag, priority, weaponName);
            }
        }

        public String FindCurrentWeapon()
        {
            return EntityInputs.InventoryAndEquipment.GetCurrentMainWeapon().WeaponObject.GetComponent<WeaponBehaviour>().Item.name;
        }
    }
}