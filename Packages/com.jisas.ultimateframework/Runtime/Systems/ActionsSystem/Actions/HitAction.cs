            ...
            try
            {
                ...
                if (useDirectionalHit)
				{
					if (damageHandler.HitDirectionVerifier(damageHandler.LastHitDirection).z > 0)
						 m_Locomotion.OverrideAnimatorController[currentStructure.overrideClip] = directionalHit.frontHit;
					else m_Locomotion.OverrideAnimatorController[currentStructure.overrideClip] = directionalHit.backHit;

					if (damageHandler.HitDirectionVerifier(damageHandler.LastHitDirection).x > 0)
						 m_Locomotion.OverrideAnimatorController[currentStructure.overrideClip] = directionalHit.leftHit;
					else m_Locomotion.OverrideAnimatorController[currentStructure.overrideClip] = directionalHit.rightHit;
				}
				else m_Locomotion.OverrideAnimatorController[currentStructure.overrideClip] = currentStructure.motion;
